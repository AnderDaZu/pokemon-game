...
describe('<PokemonGame />', () => {
    ...
    test('should render button for a new game', () => {
        ...
        const wrapper = mount(PokemonGame);
        const button = wrapper.find('[data-test-id="btn-new-game"]').text();

        expect( button ).toBe('Continuar');
    });
    ...
    test('should call the getNextRound function when the button is clicked', async () => {
        ...
        // expect( usePokemonGame().getNextRound ).toHaveBeenCalledTimes(1);

        // expect( button.text() ).toBe('Iniciar de nuevo');
    });
});